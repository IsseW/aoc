...
pub fn check_tuple(t: (u32, u32, u32)) -> bool {
    t.0.min(t.1).min(t.2) + t.0.min(t.1).max(t.1.min(t.2)).max(t.0.min(t.2)) > t.0.max(t.1).max(t.2)
}
...
pub fn solution_1(input: &str) -> String {
    input
        ...
        .filter_map(|(a, b, c)| {
            Some((
                u32::from_str_radix(a, 10).ok()?,
                u32::from_str_radix(b, 10).ok()?,
                u32::from_str_radix(c, 10).ok()?,
            ))
        })
