...
impl Iterator for LineIter {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if let Some(current) = &mut self.current {
            ...
            let line_len = ((line_vec.0 * line_vec.0 + line_vec.1 * line_vec.1) as f32).sqrt();
            let line_vec = (line_vec.0 as f32 / line_len, line_vec.1 as f32 / line_len);
            let pos = ((current.0 - self.a.0) as f32, (current.1 - self.a.1) as f32);
            let delta = *[
            ...
            .iter()
            .min_by_key(|delta| {
                let new_pos = (pos.0 + delta.0 as f32, pos.1 + delta.1 as f32);
