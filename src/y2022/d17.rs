...
fn parse(input: &str) -> impl ExactSizeIterator<Item = Dir> + '_ + Clone {
    input.bytes().map(|c| match c as char {
        '<' => Dir::Left,
        '>' => Dir::Right,
...
impl<const W: usize> BitGrid<W> {
    ...
    fn collides_with(&self, shape: Shape, height: usize) -> bool {
        ...
        o_cell >= self.rows.len()
            || (self.rows[cell] & shape1) != 0
...
fn simulate(jets: impl Iterator<Item = Dir> + Clone, num: usize) -> usize {
    ...
    let height = SHAPES
        .iter()
        .map(|shape| shape.height as usize)
        .sum::<usize>()
        * num;
...
pub fn solution_2(input: &str) -> String {
    ...
    let (cycle_height, cycle_start, cycle_end) = (0..)
        .find_map(|k| {
            let last_highest = grid.highest;
            heights.reserve(input_cycle_len);
            let x = (0..input_cycle_len)
                .map(|i| {
            ...
            match maps.binary_search_by(|cycle| {
                ...
                Ok(i) => Some((
                    last_highest - maps[i].start_height,
                    maps[i].k_start * input_cycle_len,
                    k * input_cycle_len,
                )),
                Err(i) => {
                    maps.insert(
