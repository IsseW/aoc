...
fn parse(input: &str) -> Volume<bool> {
	Volume::from_sparse(input.lines().map(|p| {
		Vec3::from(p.split(',').map(str::parse).map(Result::unwrap).collect_tuple::<(usize, usize, usize)>().unwrap())
	})).0
}
...
pub fn solution_1(input: &str) -> String {
	...
	volume.iter().map(|(p, t)| {
		if *t {
			CARDINALS.iter().filter(|n| {
				let p = p.map2(**n, |p, n| p.checked_add_signed(n));
				if let Vec3 { x: Some(x), y: Some(y), z: Some(z) } = p {
					volume.get(Vec3::new(x, y, z)).map_or(true, |p| !*p)
				} else {
					true
				}
			}).count()
...
fn floodfill(volume: &mut Volume<Cell>, pos: Vec3<usize>) {
	fn floodfill_inner(volume: &Volume<Cell>, positions: &mut HashSet<Vec3<usize>>, pos: Vec3<usize>) -> bool {
		if let Some(cell) = volume.get(pos) {
			if *cell != Cell::Water || positions.contains(&pos) {
				return true;
			}
			positions.insert(pos);
			CARDINALS.iter().filter_map(|n| {
				let p = pos.map2(*n, |p, n| p.checked_add_signed(n));
				Some(Vec3::new(p.x?, p.y?, p.z?))
			}).map(|pos| {
				floodfill_inner(volume, positions, pos)
	...
	volume.iter().map(|(p, t)| {
		if *t == Cell::Lava {
			CARDINALS.iter().filter(|n| {
				let p = p.map2(**n, |p, n| p.checked_add_signed(n));
				if let Vec3 { x: Some(x), y: Some(y), z: Some(z) } = p {
					volume.get(Vec3::new(x, y, z)).map_or(true, |p| *p == Cell::Water)
				} else {
					true
				}
			}).count()
