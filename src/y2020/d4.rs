        ...
        .filter(|(i, passport)| {
            passport
                ...
                && (passport
                    ...
                    .is_some_and(|hgt| (59..=76).contains(&hgt))
                    || passport
                        .hgt
                        .strip_suffix("cm")
                        .and_then(|s| s.parse::<u32>().ok())
                        .is_some_and(|hgt| (150..=193).contains(&hgt)))
                && passport.hcl.strip_prefix('#').is_some_and(|s| {
                    s.chars()
                ...
                && (passport.pid.len() == 9
                    && passport.pid.chars().filter(|c| c.is_ascii_digit()).count() == 9)
        })
        .count()
        .to_string()
}
